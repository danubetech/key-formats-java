...
public class secp256k1_ES256KRR_PrivateKeySigner extends PrivateKeySigner<ECKey> {

    public secp256k1_ES256KRR_PrivateKeySigner(ECKey privateKey) {

        super(privateKey, JWSAlgorithm.ES256KRR);
    }
    ...
    public byte[] sign(byte[] content) throws GeneralSecurityException {
        ...
        ECDSASignature ecdsaSignature = ec.sign(content);

        int recId = -1;

        int headerByte;
        for(headerByte = 0; headerByte < 4; ++headerByte) {
            BigInteger k = Sign.recoverFromSignature(headerByte, ecdsaSignature, content);
